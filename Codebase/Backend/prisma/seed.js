...
async function main() {
    ...
    await prisma.user.upsert({
        where: { email: 'demo@grip.local' },
        update: {},
        create: { email: 'demo@grip.local', passwordHash: demoHash, firstName: 'Demo', lastName: 'User', riskAppetite: 'moderate' },
    });
    // Enhanced AI-like description helper
    const aiDesc = (name, type, risk, yield) => {
        ...
        const riskDesc = riskDescriptions[risk] || 'moderate risk';
        const typeDesc = typeDescriptions[type] || 'investment product';
